...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        var countryAdapter = CountryAdapter(this, list,binding)
        ...
        binding.search.addTextChangedListener {
            ...
            if (it != null) {
                var fav = list
                if (status) fav = list.filter { it.status } as MutableList<Country>
                for (c in fav) {
                    if (c.name.uppercase().contains(it.toString().uppercase())) {
                        filter.add(c)
                    }
                }
                countryAdapter = CountryAdapter(this, filter,binding)
                binding.main.adapter = countryAdapter
            }
        }
        binding.favourite.setOnClickListener {
            if (!status){
                ...
                status = true
                binding.favourite.tag = 1
                var filter = list.filter { it.status }
                countryAdapter = CountryAdapter(this, filter as MutableList<Country>,binding)
                binding.main.adapter = countryAdapter
            }else {
                binding.favourite.setImageResource(R.drawable.ic_baseline_favorite_border_24)
                status = false
                binding.favourite.tag = 0
                countryAdapter = CountryAdapter(this, list, binding)
                binding.main.adapter = countryAdapter
            }
            Log.d("tag: ",binding.favourite.tag.toString())
        }
